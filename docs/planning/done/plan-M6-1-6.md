...
- Compare-Kontrakt schliesst `format_version=4`-Reports mit ein, inklusive
  ...
  `include_depth_kind`).
- Compare-Kompatibilitaetsmatrix in `spec/report-json.md` und
  verpflichtend gespiegelt in `spec/compare-matrix.md`.
...
- Eigener JSON-Output-Vertrag mit
  ...
  `format_version=1`,
  `report_type: compare`.
- Schema-Datei `spec/report-compare.schema.json`.
...
- `spec/report-compare.md` (eigener Compare-Output-Vertrag)
- `spec/report-compare.schema.json` (eigenes Compare-Schema)
- `spec/compare-matrix.md` (Compatibility-Matrix)
...
2. **Absolute-Path-Stabilisierung**: Die Teilmenge normalisierter
   `source_path`-Werte mit absoluten/rooted Pfaden wird nach Root-Anker
   gruppiert (`/`, `c:`, `//server/share`, ...). Nur innerhalb derselben
   Gruppe darf ein gemeinsamer Verzeichnispraefix unterhalb dieses
   Root-Ankers entfernt werden; relative Pfade bleiben unveraendert.
   Falls eine stabilisierte Datei mit einem bereits vorhandenen Pfad
   kollidieren wuerde, bleiben die Originalpfade erhalten. Damit bleibt
   der Compile-DB-Fallback ueber unterschiedliche Checkout-/Workspace-
   Pfade stabil, ohne Root-Grenzen oder Dateien in gemischten Compile-DBs
   zusammenzufalten.
...
Die Matrix lebt sowohl in `spec/report-json.md` als auch in einer
neuen Datei `spec/compare-matrix.md`, mit Pflichtinhalten pro
...
14. `spec/report-compare.schema.json` und Compare-
    Kompatibilitaetsmatrix in `spec/compare-matrix.md`.
15. CLI-E2E-Tests.
