...
Der DOT-Vertrag wird in `spec/report-dot.md` verbindlich dokumentiert. Dieses Dokument beschreibt:
...
`docs/user/quality.md` dokumentiert die aktive Pfadwahl pro CI-Lauf.
...
1. DOT-Vertrag in `spec/report-dot.md` festlegen, einschliesslich Graphnamen, `format_version`, Attributlexik, Label-Kuerzungsalgorithmus, Statement-/Attribut-Reihenfolge, Escaping, Node-/Edge-Kinds, Budgets, Sortier-Tie-Breakern und Weglassregeln.
...
9. `docs/user/quality.md` um die neuen DOT-Syntax-Gates und die Bootstrap-Matrix ergaenzen.

Abnahme Tranche A: alle Docker-Gates aus `README.md` und `docs/user/quality.md` gruen; Escape-/Attributlexik-Tests pruefen Leerzeichen, Anfuehrungszeichen, Backslashes und plattformtypische Pfadtrenner; `dot_graphviz_validation` failt in Docker nachweislich bei kuenstlich kaputtem DOT, `dot_python_validation` failt in nativer CI bei demselben kaputten DOT mit konkreter Fehlermeldung. Release- oder native CTest-Laeufe, die fuer AP 1.3 keinen CTest-Validator ausfuehren, muessen in `docs/user/quality.md` explizit als nicht massgeblicher DOT-Syntax-Gate-Pfad dokumentiert sein.
...
8. `docs/user/guide.md` um produktive Nutzung von `--format dot`, `--format dot --output` und ein `dot -Tsvg`-Beispiel ergaenzen.
9. `docs/user/quality.md` um die in Tranche C neu hinzukommenden DOT-Golden-, Syntax- und E2E-Gates ergaenzen.
...
- `spec/report-dot.md` vorhanden ist und den DOT-Vertrag dokumentiert.
...
- `docs/user/guide.md` die produktive DOT-Nutzung und ein `dot -Tsvg`-Beispiel beschreibt.
- `docs/user/quality.md` die neuen DOT-Syntax-, Golden- und E2E-Gates auffuehrt.
...
- Eine spaetere DOT-Vertragsversion kann konfigurierbare Budgets, Diagnostic-Knoten oder weitere Graphattribute einfuehren, muss dann aber `spec/report-dot.md`, Goldens und Tests entsprechend migrieren.
