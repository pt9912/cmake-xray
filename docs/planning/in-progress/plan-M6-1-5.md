...
- A.4 (JSON- und DOT-Adapter): **erledigt**. Lead-Commit
  `4d5d1ff` (A.4 coverage tidy-up json brace-init). Tranche-Hashes:
  `53b6e48` (Step 14 JSON adapter v5 emits analysis_configuration +
  analysis_section_states), `995bfc6` (Step 15 DOT adapter v5 emits
  seven new analysis_configuration graph-attributes), `8a242fb`
  (TargetGraphStatus::disabled-Mapping in Console/Markdown/HTML als
  A.4/A.5-Folgearbeit zu Step 11), `b470b53`
  (kReportFormatVersion-Pin in zwei Adapter-/Service-Tests von 4
  auf 5), `62fd1cd` (Step 16 report-dot.md v5), `84eec15` (Step 17
  Goldens-Regen 113 Files via run_e2e_artifacts/smoke/verbosity-
  Wrapper), `771eedb` (HOSTPATH-Normalisierung der DOT-Goldens),
  `1ce4c7c` (docs/examples-Regen via generation-spec.json +
  Verbosity-Doctest-Goldens v4->v5 + impact_absolute_windows.json
  format_version-Bump) und `4d5d1ff` (Coverage-Tidy-Up
  render_analysis_configuration brace-init splitten). Alle drei
  Docker-Gates auf `4d5d1ff` gruen: docker-test 41/41, coverage
  100%, quality clean.

  Empty-Section-Suppression fuer `disabled`/`not_loaded` in JSON
  und DOT (plan §572-594) ist in dieser Tranche bewusst nicht
  umgesetzt; alle aktuellen Tests fahren mit `--analysis all`, also
  bleibt die Suppression eine Folgearbeit fuer A.5/A.6, sobald
  Adapter-Tests einen non-default `--analysis`-Lauf einfuehren.
- A.5 (HTML-, Markdown- und Console-Adapter): **erledigt**. Lead-
  Commit `95d2db2` (Step 20 Goldens fuer Console/Markdown/HTML).
  Tranche-Hashes: `12d327c` (Step 18a Console adapter v5 emits
  Analysis Configuration + Section states blocks), `4ab303e`
  (Step 18b Markdown adapter v5 emits Analysis Configuration section
  and Section States table), `f9a3669` (Step 18c HTML adapter v5 emits
  Analysis Configuration section, section-state badges and unified
...
- A.6 (Audit-Pass): **lokal erledigt**. Der Audit schliesst die
  ...
  deterministischen Empty-Section-Strukturen; DOT unterdrueckt Knoten
  und Kanten deaktivierter bzw. nicht geladener Sections, behaelt aber
  die `graph_*`-Konfigurationsattribute. Lokale Gates auf diesem
  Arbeitsstand gruen: `make docker-test` (41/41), `make coverage-gate
  COVERAGE_THRESHOLD=100` (41/41, Coverage 100%), `make quality-gate`
  (clang-tidy 0, lizard 0) und `git diff --check`.
