...
auf `2`. Alle Format-Vertrags-Dokumente (`spec/report-json.md`,
`spec/report-json.schema.json`, `spec/report-dot.md`, `spec/report-html.md`)
...
- **`raw_id`-Escaping und Truncation**: Beide Attribute (`label` und
  ...
  - **Stringescaping bleibt aktiv fuer beide Attribute**: Backslash zu
    ...
    verweist auf den M5-DOT-Vertrag in `spec/report-dot.md`.
...
- `graph_target_graph_status` (string-quoted): `not_loaded`, `loaded`,
  ...
  (siehe `spec/report-dot.md` `graph_node_limit`, `graph_edge_limit`,
...
3. `spec/report-json.md` auf v2 aktualisieren.
...
14. CLI-E2E-Tests vollstaendig durchlaufen lassen, byte-stabil gegen
    Goldens vergleichen.
15. Docker-Gates ausfuehren (siehe `docs/user/quality.md`); Tranche gilt erst
...
- **A.1 Schema, JSON-Adapter und JSON-Goldens (atomar)**:
  ...
  `spec/report-json.md` auf v2 aktualisiert, JSON-Adapter implementiert
...
- **A.2 DOT-Adapter und DOT-Goldens**: DOT-Adapter implementiert
  ...
  synthetische `external_target`-Knoten), `spec/report-dot.md` auf v2
...
- **A.3 HTML-, Markdown- und Console-Adapter**: HTML-Adapter,
  ...
  `spec/report-html.md` auf v2 aktualisiert. Goldens fuer alle drei
...
- **A.4 Audit-Pass**: Plan-Test-Liste aus `Tests`-Sektion gegen Ist-Stand
  ...
  und in `docs/planning/plan-M6.md` aktualisieren.
...
- A.4 (Audit-Pass: Plan-Test-Liste gegen Ist-Stand verifiziert,
  ...
  `git log -- docs/planning/plan-M6-1-2.md`. Audit-Befunde: HTML/Markdown/
...
Docker-Gate-Lauf gemaess `docs/user/quality.md` auf dem A.4-Commit lokal gruen:
...
- Compile-DB-only-Goldens fuer Console und Markdown byte-stabil gegenueber
  M5 bleiben;
- der Docker-Gate-Lauf gemaess `docs/user/quality.md` (Tranchen-Gate fuer M6
  AP 1.2) gruen ist;
