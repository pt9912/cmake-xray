...
Der HTML-Vertrag wird in `spec/report-html.md` verbindlich dokumentiert. Dieses Dokument beschreibt:
...
- Farbkontrast fuer Text und Status-Badges muss mindestens WCAG-AA-nahe Basiswerte erreichen; AP 1.4 dokumentiert die konkreten Farbwerte in `spec/report-html.md`.
...
- Attribute werden in der Reihenfolge geschrieben, in der sie in `spec/report-html.md` je Elementtyp dokumentiert sind.
...
- Zellenreihenfolge ist in `spec/report-html.md` dokumentiert und golden-getestet.
...
- Innerhalb gleicher Impact-Art wird nach `source_path`, `directory`, `unique_key` sortiert, sofern das Modell keine bereits stabile Reihenfolge garantiert; `spec/report-html.md` legt den finalen Pfad fest und Goldens pruefen ihn.
...
Die Umsetzung erfolgt in drei verbindlichen Tranchen plus einer optionalen Haertungstranche. Jede Tranche endet mit einem vollstaendigen Lauf der Docker-Gates aus `README.md` ("Tests und Quality Gates") und `docs/user/quality.md`. Die globalen Abnahmekriterien dieses Plans gelten zusaetzlich am Ende von Tranche C.
...
1. HTML-Vertrag in `spec/report-html.md` festlegen, einschliesslich Dokumentgeruest, CSS, Escaping, Abschnittsreihenfolge, Tabellenstruktur, Leersaetzen, Accessibility-Basis und Golden-Regeln.
...
7. `docs/user/quality.md` um den neuen HTML-Golden-/Struktur-Testumfang ergaenzen.
...
- [x] `spec/report-html.md` dokumentiert den HTML-Vertrag.
...
- [x] `docs/user/quality.md` beschreibt den HTML-Testumfang.
- [x] Docker-Gates aus `README.md` und `docs/user/quality.md` sind gruen.
...
Tranche C ist intern in drei Sub-Tranchen C.1, C.2 und C.3 gegliedert. Die Sub-Tranchen sind ausschliesslich Implementierungsreihenfolge, kein Release-Schnitt: alle drei Sub-Tranchen landen im selben Tranche-C-Commit-Set und werden gemeinsam ueber die Docker-Gates aus `README.md` und `docs/user/quality.md` abgenommen. "Abnahme Tranche C" und "Definition of Done Tranche C" am Ende dieses Abschnitts gelten am Ende von C.3 fuer das gesamte Tranche-C-Set.

Die AP-1.2-/1.3-Lehre `README-Pflicht in Tranche C` bleibt verbindlich: Beispielberichte unter `docs/examples/`, README-Update, `docs/user/guide.md`-Update und `docs/user/quality.md`-Update gehoeren in dasselbe Commit-Set wie die HTML-Goldens und werden nicht als separater Folge-Commit nachgezogen.
...
3. `docs/user/guide.md` um produktive Nutzung von `--format html` und `--format html --output` ergaenzen.
4. `docs/user/quality.md` um die in Tranche C neu hinzukommenden HTML-Golden-, Struktur- und E2E-Gates ergaenzen.
...
- README- und `docs/user/quality.md`-Aktualisierung muessen mit demselben Commit-Set wie die Goldens und Beispielberichte landen, sonst greift die AP-1.2-/1.3-Lehre und die Doku-Aktualisierung muss als separater Folge-Commit nachgezogen werden.
...
- Lizard-CCN-/Funktionslaengen-Grenzen koennen durch grosse Render-Funktionen im Adapter knapp werden; falls Refactoring noetig wird, bleibt der HTML-Vertrag aus `spec/report-html.md` unveraendert und Goldens aus C.1 bleiben byte-stabil.
...
- [x] `docs/user/guide.md` dokumentiert produktive Nutzung von `--format html` und `--format html --output`.
- [x] `docs/user/quality.md` listet die neuen HTML-Gates aus Tranche C.
...
Abnahme Tranche C: alle Docker-Gates aus `README.md` und `docs/user/quality.md` gruen; `e2e_binary` gruen; alle HTML-Goldens sind byte-stabil; Escaping-, Struktur-, Accessibility-Basis-, Top-Limit- und Leersatzvertraege sind durch Goldens und Adaptertests abgedeckt; globale Abnahmekriterien dieses Plans erfuellt.
...
- [x] `README.md`, `docs/user/guide.md` und `docs/user/quality.md` sind aktualisiert.
...
- [x] Keine Haertung veraendert den dokumentierten AP-1.4-Vertrag ohne Update von `spec/report-html.md`.
- [x] Docker-Gates aus `README.md` und `docs/user/quality.md` bleiben gruen.
...
- `spec/report-html.md` vorhanden ist und den HTML-Vertrag dokumentiert.
...
- `README.md`, `docs/user/guide.md` und `docs/user/quality.md` die produktive HTML-Nutzung und die neuen Gates auffuehren.
...
- Eine spaetere HTML-Vertragsversion kann optional clientseitige Features, weitere Accessibility-Checks oder Diagnostic-Visualisierungen einfuehren, muss dann aber `spec/report-html.md`, Goldens und Tests entsprechend migrieren.
