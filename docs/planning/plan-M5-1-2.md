...
Nach AP 1.2 kann `cmake-xray analyze --format json` und `cmake-xray impact --format json` produktiv genutzt werden. Die Ausgabe folgt dem in `spec/report-json.md` dokumentierten Vertrag und verwendet die Formatversion aus AP 1.1.
...
- Dokumentierter JSON-Vertrag in `spec/report-json.md`.
- Maschinenlesbares JSON Schema in `spec/report-json.schema.json`.
...
Der JSON-Vertrag ist in `spec/report-json.md` verbindlich dokumentiert. Dieses Dokument beschreibt fuer alle Felder:
...
- `changed_file_source` ist einer der in `spec/report-json.md` dokumentierten Enum-Werte.
...
- `spec/report-json.schema.json` verwendet JSON Schema Draft 2020-12 und enthaelt `$schema: "https://json-schema.org/draft/2020-12/schema"`.
...
- `spec/report-json.schema.json` muss `format_version` mit `const: 1` absichern.
...
`spec/report-json.md` und `spec/report-json.schema.json` entstehen vor oder im selben Umsetzungsschritt wie `JsonReportAdapter`. Der Adapter darf nicht ohne verbindlichen Item-Vertrag fuer Ranking-, Hotspot-, Diagnostic-, Translation-Unit- und Target-Objekte landen.
...
1. JSON-Vertrag in `spec/report-json.md` festlegen, einschliesslich Typ, Pflichtstatus, Enum-Werten, Nullability, Sortier-Tie-Breakern und der Ausnahme `unresolved_file_api_source_root` aus dem JSON-Enum.
2. `spec/report-json.schema.json` (Draft 2020-12) anlegen, mit `additionalProperties: false` auf jedem Vertragsobjekt und `format_version: {const: 1}`.
...
8. `docs/user/quality.md` um die neuen Schema- und Versionsgates ergaenzen.
...
6. `docs/user/guide.md` um die produktive Nutzung von `--format json` und `--format json --output` ergaenzen; `docs/user/quality.md` um die in Tranche C neu hinzukommenden Golden- und E2E-Gates ergaenzen.
...
- CTest-integrierter Schema-Validierungstest validiert alle JSON-Goldens gegen `spec/report-json.schema.json`.
...
- Dieselben erzeugten Reports werden gegen `spec/report-json.schema.json` validiert.
...
- `spec/report-json.md` und `spec/report-json.schema.json` vorhanden, konsistent und in Tests eingebunden sind.
...
- `docs/user/guide.md` die produktive JSON-Nutzung beschreibt und `docs/user/quality.md` die neuen JSON-Schema-, Golden- und E2E-Gates auffuehrt.
...
- Eine spaetere JSON-Formatversion kann neue Felder oder JSON-Fehlerobjekte einfuehren, muss dann aber `format_version` erhoehen und `spec/report-json.md` sowie `spec/report-json.schema.json` entsprechend migrieren.
