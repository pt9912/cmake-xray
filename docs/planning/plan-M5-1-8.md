...
- `docs/user/guide.md` mit praktischen Aufrufen fuer alle M5-Formate
- `docs/user/releasing.md` mit finalem Artefaktumfang und Release-Verifikation
- `docs/user/quality.md` mit M5-Testumfang und Plattform-Smokes
- `docs/user/performance.md` mit File-API-Performance-Baseline als erster Folge-Meilenstein nach M4
...
- Unit-/Integrationstests fuer den Atomic-Replace-Wrapper, die existierende Ziele ohne vorheriges Loeschen ersetzen und simulierte Fehlerpfade mit intakter alter Datei pruefen; Windows-API-Contract-Tests fuer die gewaehlte `ReplaceFileW`-/`MoveFileExW`-Semantik sind plattformunabhaengig abzusichern, echte Windows-Host-Tests sind nur bei Plattformstatus `validated_smoke` verpflichtend und werden andernfalls als konkrete `known_limited`-Einschraenkung dokumentiert
...
- Dokumentations- und Stream-Vertrag-Sync fuer `spec/report-json.md`, `spec/report-dot.md` und `spec/report-html.md`: erfolgreiche Normal- und Quiet-Artefaktausgaben bleiben ohne `--output` stdout-only und mit `--output` stdout/stderr-leer; erfolgreiche Verbose-Artefaktausgaben duerfen ausschliesslich Zusatzdiagnostik auf `stderr` ausgeben und Report-stdout beziehungsweise Reportdatei nicht veraendern
...
- CLI-/E2E-Negativtests fuer File-API-only-Impact-Laeufe mit `changed_file_source=unresolved_file_api_source_root`: `impact --format json` und `impact --format json --output <path>` muessen nonzero enden, Text auf `stderr` schreiben, kein stdout-JSON ausgeben und keine Zieldatei erzeugen, damit der JSON-v1-Vertrag "Textfehler ohne JSON-Report" nicht nur auf Schema-/Adapterebene abgesichert ist
...
- Doku-Gate fuer Nutzeraufrufe: `README.md`, `docs/user/guide.md`, `docs/user/releasing.md` und `docs/examples/` duerfen keine interne Build-Pfad-Nutzung wie `./build/cmake-xray` voraussetzen und beschreiben Release-Artefakte, Container oder installierte Binary-Pfade als regulaere Nutzerwege; `docs/user/performance.md` ist als Entwickler- und Messdokument ausgenommen, muss interne Build-Pfade aber als reproduzierbare Messkommandos kennzeichnen und darf sie nicht als normale Nutzeraufrufe darstellen
- Validierung, dass JSON syntaktisch gueltig ist, `format_version` enthaelt, den Pflichtfeld-, Typ-, Enum-, Nullability- und Array-Regeln aus `spec/report-json.md` entspricht und gegen `spec/report-json.schema.json` validiert
...
- Validierung, dass `docs/examples/` keine driftenden Kopien enthaelt: Markdown-, HTML-, JSON- und DOT-Beispiele werden aus validierten Goldens generiert und in CI gegen den aktuellen Generatorausgang verglichen oder ueber das in `tests/CMakeLists.txt` registrierte CTest-Gate `doc_examples_validation` mit `tests/validate_doc_examples.py` und `docs/examples/manifest.txt` gegen Golden-Outputs byteverglichen; JSON, DOT und HTML durchlaufen zusaetzlich die jeweiligen Schema-, Syntax- und Struktur-Manifeste, aber diese Formatvalidierung ersetzt den Drift-Vergleich nicht
...
8. Abnahmedokumente und `docs/user/quality.md` auf finalen Status aktualisieren

### Liefer-Stand

Die AP-1.8-Auslieferung ist in fuenf Sub-Tranchen A.1 bis A.5 geschnitten,
analog zum Liefer-Stand-Vertrag aus `docs/planning/plan-M5-1-6.md` und
`docs/planning/plan-M5-1-7.md`:
...
- A.3 ausgeliefert in commit `2835017` ("feat: deliver M5 AP 1.8 A.3 versioned File-API performance fixtures"): `tests/reference/scale_*/CMakeLists.txt` plus versionierte Reply-Daten unter `build/.cmake/api/v1/reply/`, `tests/reference/generate_file_api_fixtures.sh` als Regenerator, `tests/reference/file-api-performance-manifest.json` mit CMake-Version, Generator und Reply-Pfaden, neue File-API-Baseline-Sektion in `docs/user/performance.md`.
- A.4 ausgeliefert in commit `d310d1d` ("feat: deliver M5 AP 1.8 A.4 v1.2.0 docs sweep and changelog"): `CHANGELOG.md` v1.2.0-Sektion mit Added/Changed/Fixed-Gruppen plus Plattformstatus-Tabelle und expliziter "no breaking changes"-Kennzeichnung; README-Intro entwirrt (HTML in der Format-Liste); `docs/user/releasing.md` Recovery-only-Framing fuer manuelle `gh`-Aufrufe mit Ist-/Soll-Abgleich-Pflicht.
- A.5 (Audit-Pass): Plan-Test-Liste aus `1.8 Scope` (46 Bullets) gegen Ist-Stand verifiziert (commit `1ef1db8`, "docs: deliver M5 AP 1.8 A.5 audit pass and Liefer-Stand block"). Zwei reale Luecken vom ersten Audit-Lauf nicht gesehen, in zwei Fixup-Commits geschlossen:
  - commit `8604b79` ("fix: close M5 AP 1.8 A.5 audit gap on JSON unresolved render precondition") schliesst Bullet 22: der fehlende JSON-CLI-Negativtest fuer `impact --format json` plus `--output`-Variante bei `changed_file_source=unresolved_file_api_source_root` war unter den DOT/HTML-Tests gerollt, der Spot-Check zeigte aber, dass der JSON-CLI-Pfad tatsaechlich exit 0 und eine JSON-Datei lieferte. `format_rejects_unresolved_file_api` enthaelt jetzt JSON neben DOT/HTML, zwei neue CLI-Tests in `tests/e2e/test_cli.cpp` pinnen das Verhalten; der existierende Verbose-Stderr-Test in `tests/e2e/test_cli_verbosity.cpp` ist auf Markdown umgezogen.
  - commit `f211873` ("fix: close M5 AP 1.8 docs/examples coverage gap on JSON and DOT") schliesst die Plan-Z.10-Pflicht "Markdown-, HTML-, JSON- und DOT-Ausgaben in docs/examples/": JSON- und DOT-Beispiele fehlten komplett (8 Dateien fuer analyze/impact x mit/ohne Targets). Alle acht Beispiele sind aus den existierenden Markdown-/HTML-Fixturen via `cmake-xray:test` erzeugt, in `docs/examples/manifest.txt` mit SHA-256 fixiert und ueber `tests/validate_doc_examples.py` zusaetzlich gegen `spec/report-json.schema.json` bzw. `tests/validate_dot_reports.py` validiert. README.md und docs/user/guide.md verlinken die neuen Artefakte.
  - commit `b2b124e` ("fix: align docs/user/guide.md OCI tag references with the AP M5-1.6 release contract") schliesst eine vierte real-world Doku-Inkonsistenz: `docs/user/guide.md` zeigte den OCI-Tag als `ghcr.io/pt9912/cmake-xray:vX.Y.Z` (mit fuehrendem `v`) und behauptete "Container werden mit dem Git-Tag wie vX.Y.Z markiert". Der AP-1.6-Vertrag pinnt den OCI-Tag aber an die App-Version *ohne* `v` (`scripts/oci-image-publish.sh` plus `release.yml`); README.md war bereits korrekt. Alle drei `:vX.Y.Z`-Stellen plus die erklaerende Prosa sind jetzt auf die README-Form aligned.
  - commit `7c678ca` ("fix: close M5 AP 1.8 docs/examples drift gate against current generator output") schliesst Bullet 68: die A.2-Implementierung pinnte SHA-256 der Beispieldateien gegen sich selbst, deckte aber den vom Plan geforderten Vergleich gegen "aktuellen Generatorausgang" oder "Golden-Outputs" nicht. Neuer `docs/examples/generation-spec.json` listet die `cmake-xray`-Args pro Beispiel, `tests/validate_doc_examples.py` faehrt mit `--binary <path>` jeden Spec-Eintrag aus und byte-diff't gegen die committete Datei; der CTest-Eintrag `doc_examples_validation` reicht `$<TARGET_FILE:cmake-xray>` durch. Zwei Spec-Mismatches gegen den Bestand sind beim Bring-up gefunden und korrigiert worden: `analyze-console-targets.txt` stammt aus `m4/file_api_only/build` (nicht `partial_targets`), und `impact-report-targets.html` wurde regeneriert, weil die existierende Datei ohne `--cmake-file-api` gerendert war (`target_metadata: not_loaded`) und damit zum Markdown-Geschwister inkonsistent.

  Bullet 42 (Plattformstatus-Hochstufung auf `validated_smoke` fuer macOS/Windows) bleibt plan-konform `known_limited`, weil die externe Branch-Protection-Verankerung und der auditierte gruene CI-Lauf Repo-Konfiguration sind und nicht innerhalb von AP 1.8 hergestellt werden koennen. Die externen Bedingungen sind in `README.md`, `docs/user/quality.md` und `docs/user/releasing.md` als verbleibende Hochstufungs-Voraussetzung dokumentiert.
...
- JSON- und DOT-Goldens entsprechen `spec/report-json.md` bzw. `spec/report-dot.md` sowie den validierten Schema-/Syntax-Gates
...
- README, Guide, Releasing-Doku und Beispiele beschreiben Nutzeraufrufe ohne interne Build-Pfade wie `./build/cmake-xray`; `docs/user/performance.md` ist als Entwickler- und Messdokument ausgenommen, muss interne Build-Pfade aber als reproduzierbare Messkommandos kennzeichnen und darf sie nicht als normale Nutzeraufrufe darstellen
...
- `docs/user/releasing.md` enthaelt im Normalpfad kein `gh release upload --clobber` und keinen Wildcard-Asset-Upload; manuelle externe Publikation ist nur als Recovery-Pfad mit vorherigem Ist-/Soll-Abgleich von Release, Assets, Checksums und OCI-Digests dokumentiert
...
- Weiterfuehrende Performance-Iteration in `docs/user/performance.md` nach den ersten M5-Baselines
