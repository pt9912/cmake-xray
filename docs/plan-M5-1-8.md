...
- A.5 (Audit-Pass): Plan-Test-Liste aus `1.8 Scope` (46 Bullets) gegen Ist-Stand verifiziert (commit `1ef1db8`, "docs: deliver M5 AP 1.8 A.5 audit pass and Liefer-Stand block"). Audit-Fixup in commit `8604b79` ("fix: close M5 AP 1.8 A.5 audit gap on JSON unresolved render precondition") schliesst Bullet 22: das Audit hatte den fehlenden JSON-CLI-Negativtest fuer `impact --format json` plus `--output`-Variante (`changed_file_source=unresolved_file_api_source_root`) zunaechst unter den existierenden DOT/HTML-Tests gerollt; der Spot-Check zeigte aber, dass der JSON-CLI-Pfad tatsaechlich exit 0 und eine geschriebene JSON-Datei lieferte. `format_rejects_unresolved_file_api` enthaelt jetzt JSON neben DOT/HTML, zwei neue CLI-Tests in `tests/e2e/test_cli.cpp` pinnen das Verhalten, und der bestehende Verbose-Stderr-Test in `tests/e2e/test_cli_verbosity.cpp` ist auf Markdown umgezogen, weil JSON nun ebenfalls vor der Verbose-Stderr-Emission abbricht. Bullet 42 (Plattformstatus-Hochstufung auf `validated_smoke` fuer macOS/Windows) bleibt plan-konform `known_limited`, weil die externe Branch-Protection-Verankerung und der auditierte gruene CI-Lauf Repo-Konfiguration sind und nicht innerhalb von AP 1.8 hergestellt werden koennen. Die externen Bedingungen sind in `README.md`, `docs/quality.md` und `docs/releasing.md` als verbleibende Hochstufungs-Voraussetzung dokumentiert.
