# Plan M4 - Target-Metadaten und initiale Target-Sicht (`v1.1.0`)
...
### 0.1 Zweck
Dieser Plan beschreibt die konkreten Schritte fuer Milestone M4 (`v1.1.0`). Ziel ist der erste Ausbau nach dem MVP: `cmake-xray` soll optionale Target-Metadaten aus der CMake File API einlesen koennen, Translation-Unit-Beobachtungen deterministisch Targets zuordnen und diese Zusatzsicht in `analyze` und `impact` fuer Konsole und Markdown nutzbar machen.

M4 ist bewusst kein Sammel-Meilenstein fuer alle Phase-4-Erweiterungen. Der Fokus liegt auf `F-19` und `F-24`, also auf TU-zu-Target-Zuordnung und targetbezogener Impact-Ausgabe. Bestehende M3-Aufrufe ohne Target-Metadaten sollen fachlich und in ihrer Ausgabeform rueckwaertskompatibel bleiben.
...
- `analyze` bei geladener Target-Metadatenlage die bestehenden TU- und Hotspot-Ausgaben um sichtbare Target-Zuordnungen erweitert, ohne die M3-Standardausgabe ohne Zusatzmetadaten zu veraendern
- `impact` bei geladener Target-Metadatenlage zusaetzlich betroffene Targets ausgibt; direkte Evidenz aus direkt betroffenen Translation Units und heuristische Evidenz aus include-basiert betroffenen Translation Units bleiben unterscheidbar
- unvollstaendige oder nur teilweise verwertbare Target-Metadaten als Diagnostics sichtbar werden, ohne die Kernanalyse zu maskieren
- explizit angegebene, aber nicht lesbare oder syntaktisch ungueltige Target-Metadaten mit klarer Fehlermeldung und konsistenter Exit-Code-Semantik behandelt werden
- Adapter-, Kern-, Reporter- und End-to-End-Tests die Pfade mit und ohne Target-Metadaten sowie regressionskritische Faelle fuer Mehrfachzuordnungen, partielle Metadaten und stabile Sortierung absichern

Relevante Kennungen: `F-19`, `F-24`, `F-26`, `F-27`, `F-31`, `F-32`, `F-33`, `F-34`, `F-35`, `F-36`, `NF-10`, `NF-14`, `NF-15`, `NF-18`, `NF-19`, `S-02`

### 0.3 Abgrenzung
Bestandteil von M4 sind:

- ein echter, strukturierter `TargetMetadataPort` statt des bisherigen Bool-Platzhalters
- ein `CmakeFileApiAdapter` als erste konkrete `TargetMetadataPort`-Implementierung
- TU-zu-Target-Zuordnung fuer einzelne Beobachtungen aus der Compile-Datenbank
- targetbezogene Erweiterungen der bestehenden `analyze`- und `impact`-Ergebnisse fuer Konsole und Markdown
- Diagnostics fuer fehlende, partielle oder mehrdeutige Target-Metadaten
...
Diese Abgrenzung setzt die Architekturentscheidung aus [architecture.md](./architecture.md) um: Phase 4 beginnt mit `F-19` und `F-24`; weitergehende Target-Graph-Auswertungen und weitere Report-Formate folgen erst danach.

## 1. Arbeitspakete

### 1.1 Target-Metadaten-Vertraege und Kernmodelle schaerfen

Der bestehende `TargetMetadataPort` ist fuer M4 nur ein Platzhalter. Fuer die erste echte Target-Sicht wird ein strukturierter Vertrag benoetigt, der zwischen vorhandenen, fehlenden und nur teilweise verwertbaren Metadaten unterscheiden kann.
...
- ein Ergebnisobjekt fuer geladene Target-Metadaten inklusive Diagnostics
...
- substanzielle Ueberarbeitung von `src/hexagon/ports/driven/target_metadata_port.h`

**Ergebnis**: Der Kern besitzt einen belastbaren, diagnostikfaehigen Vertrag fuer optionale Target-Metadaten, ohne bereits einen vollstaendigen Target-Graphen einzufuehren.

### 1.2 `CmakeFileApiAdapter` fuer CMake-Reply-Daten umsetzen

Als erster M4-Adapter fuer `S-02` wird die CMake File API angebunden. Der Adapter soll Reply-Daten lesen, daraus Targets und ihre Quellzuordnungen extrahieren und diese Informationen in Kernmodelle ueberfuehren.
...
- relevant fuer M4 sind vor allem Target-Name, Target-Typ und Source-Dateien; tiefergehende Graph-Informationen duerfen intern gelesen, muessen aber noch nicht ausgewertet werden
...
- Quellen, die in den Target-Metadaten vorhanden sind, aber nicht zu geladenen Compile-Database-Beobachtungen passen
...
- die Zuordnung laeuft immer von einer konkreten Compile-Database-Beobachtung zur Target-Menge; doppelte Beobachtungen bleiben getrennt
...
**Ergebnis**: `cmake-xray` kann eine reale, versionierbare Quelle fuer Target-Metadaten lesen, ohne den Kern an CMake-spezifische JSON-Strukturen zu koppeln.

### 1.3 Projekt- und Impact-Analyse um Target-Sicht erweitern

Die neue Eingabequelle ist erst nutzbar, wenn Projekt- und Impact-Analyse sie in ihre Ergebnisobjekte integrieren. M4 erweitert deshalb die bestehenden Analysekern-Pfade um eine optionale zweite Datengrundlage.
...
- ohne geladene Target-Metadaten bleiben die Ergebnisobjekte fachlich auf M3-Niveau; neue Felder koennen leer sein, duerfen aber keine geaenderte Standardausgabe erzwingen
...
**Ergebnis**: Die bestehende Analyse liefert optional targetbezogenen Mehrwert, ohne ihren M3-Kernpfad funktional neu zu definieren.
...
- ohne Target-Metadaten bleibt die M3-Ausgabe fuer identische Eingaben bytegleich
- mit Target-Metadaten werden bestehende TU-Zeilen in Konsole und Markdown um eine deterministisch sortierte Target-Anzeige erweitert, zum Beispiel als Suffix ` [targets: app, core]`
...
- der bestehende M3-Vertrag bleibt fuer Aufrufe ohne Target-Metadaten unveraendert
- `analyze` ergaenzt in der Uebersicht genau dann Target-Zeilen, wenn Target-Metadaten geladen wurden, mindestens:
...
**Ergebnis**: Target-Metadaten werden in den bestehenden Report-Pfaden sichtbar, ohne fuer M4 einen dritten Analysemodus oder ein neues Ausgabeformat einzufuehren.

### 1.5 CLI, Eingabepfad und Fehlersemantik fuer optionale Target-Metadaten festlegen
...
- partielle, aber parsebare Target-Metadaten liefern Exit-Code `0` und werden ueber Diagnostics erklaert
...
M4 fuehrt mit der CMake File API eine zweite reale Datengrundlage ein. Damit steigen sowohl die Anzahl der Randfaelle als auch das Risiko, dass Target-Zuordnungen durch Pfad- oder Sortierdetails instabil werden.
...
- Regressionstests, die bestaetigen, dass M3-Ausgaben ohne Target-Metadaten unveraendert bleiben

Fuer Referenzdaten gilt in M4:

- neue Fixtures unter `tests/e2e/testdata/m4/` enthalten jeweils `compile_commands.json`, relevante Quelldateien und eine versionierte minimale File-API-Reply-Struktur
...
**Ergebnis**: Nutzungsdokumentation, Beispielausgaben und Release-Hinweise bilden den ersten post-MVP-Ausbau fuer Target-Metadaten konsistent ab.
...
| Driven Port | `src/hexagon/ports/driven/target_metadata_port.h` |
...
|---|---|---|
| 1a | 1.1 Target-Metadaten-Vertraege und Kernmodelle schaerfen | M3 |
...
|---|---|
| Target-Metadaten-Vertrag und CMake-File-API-Eingabe | `F-19`, `F-35`, `S-02`, `NF-14` |
...
|---|---|---|
| Fokus von M4 | M4 bleibt auf `F-19` und `F-24` fokussiert; weitergehende Target-Graph-Analysen und weitere Ausgabeformate bleiben ausserhalb dieses Milestones | 0.3 |
| Quelle fuer Target-Metadaten | Erster Adapter ist die CMake File API; gelesen werden nur vorhandene Reply-Daten, ohne CMake-Lauf oder Query-Erzeugung | AP 1.2 |
| CLI fuer Metadaten | `analyze` und `impact` erhalten die optionale Angabe `--cmake-file-api <path>`; es gibt keine implizite Auto-Erkennung aus `--compile-commands` | AP 1.5 |
