...
Dieses Dokument ist der verbindliche Vertrag fuer den maschinenlesbaren JSON-Report von `cmake-xray`. Der Vertrag wird durch `docs/report-json.schema.json` formal abgesichert, durch Goldens unter `tests/e2e/testdata/m5/json-reports/` byte-stabil festgehalten und durch CTest-Gates in jeder Build-Matrix gegen die C++-Konstante `xray::hexagon::model::kReportFormatVersion` abgeglichen.
