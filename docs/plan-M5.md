...
- `ReportInputs` dokumentiert die Provenienz ueber strukturierte Zusatzfelder wie `compile_database_source`, `cmake_file_api_source` und `changed_file_source`; `compile_database_source` verwendet fuer M5 die Enum-Werte `cli`, `default`, `not_provided`, `cmake_file_api_source` verwendet `cli`, `not_provided`, und `changed_file_source` verwendet `compile_database_directory`, `file_api_source_root`, `not_provided`, damit File-API-only- und Mixed-Input-Laeufe eindeutig auswertbar bleiben
...
- DOT-Golden-Tests fuer gleichzeitige `context_limit`-, `node_limit`- und `edge_limit`-Kuerzung, damit `context_total_count`, `context_returned_count` und mehrfach referenzierte Kontext-Translation-Units deterministisch gezaehlt werden
...
- JSON-Schema-/Golden-Tests fuer alle `*_source`-Enums: `compile_database_source=cli|default|not_provided`, `cmake_file_api_source=cli|not_provided` und `changed_file_source=compile_database_directory|file_api_source_root|not_provided`; absolute-Pfad-Goldens muessen synthetische, fixture-stabile Pfade verwenden
