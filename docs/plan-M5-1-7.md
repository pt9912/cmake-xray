...
- Statushochstufung am 2026-04-30: macOS arm64 und Windows x86_64 sind ab heute als `validated_smoke` dokumentiert. Beide externen Bedingungen aus dem Plattformstatus-Vertrag sind erfuellt: (a) Branch-Protection auf `main` verankert die fuenf Required Checks (`Native (linux-x86_64)`, `Native (macos-arm64)`, `Native (windows-x86_64)`, `Docker Runtime Build`, `docs/examples Host-Portability`) ueber `gh api -X PUT repos/pt9912/cmake-xray/branches/main/protection`; (b) build.yml run 25153798452 (commit `7be4829`, post-protection, alle fuenf Required Checks gruen) ist der auditierbare CI-Lauf auf den Plattform-Runnern. Die Statushochstufung wurde in `README.md`, `docs/guide.md`, `docs/releasing.md` und `docs/quality.md` byte-konsistent gespiegelt; CHANGELOG-Eintrag unter `[Unreleased]` "Changed" referenziert den Audit-Anker. v1.2.0's `[1.2.0]`-Plattform-Tabelle bleibt als historisches Release-Record unveraendert.
